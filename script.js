...
function addTransactionDOM(transaction) {
  ...
  loan_item.innerHTML = `
  ${transaction.customername}-${transaction.bank} <span> -$ ${Math.abs(transaction.loan)}</span>`;
...
*/
  ...
  CustomerBalance = transaction.deposit - transaction.loan;

  if (CustomerBalance >= 0) {
      bal_item.classList.add('plus');
      bal_item.innerHTML = `${transaction.customername}-${transaction.bank} <span> $ ${CustomerBalance}</span>`;

  }
  else {
      bal_item.classList.add('minus');
      bal_item.innerHTML = `${transaction.customername}-${transaction.bank} <span> -$ ${Math.abs(CustomerBalance)}</span>`;
